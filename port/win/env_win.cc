...
static const size_t kSectorSize =
    512;  // Sector size used when physical sector size could not be obtained
          // from device.
...
Status WinEnvIO::OpenWritableFile(const std::string& fname,
  ...
  HANDLE hFile = 0;
  {
    ...
    hFile = RX_CreateFile(
        ...
        creation_disposition,  // Posix env says (reopen) ? (O_CREATE |
...
void WinEnvThreads::StartThread(void (*function)(void* arg), void* arg) {
  ...
  state->arg = arg;
  try {
    TERARKDB_NAMESPACE::port::WindowsThread th(&StartThreadWrapper,
                                               state.get());
    state.release();
...
Status WinEnv::AreFilesSame(const std::string& first, const std::string& second,
                            bool* res) {
