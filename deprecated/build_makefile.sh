...
#
#   This build script is depprecated!!! please use default build.sh (CMake version)
#
...
WITH_BYTEDANCE_METRICS=1
if [ "$ENABLE_METRICS_BUILD" == "0" ]; then
  WITH_BYTEDANCE_METRICS=0
fi
...
make LINK_TERARK=static \
     ...
     WITH_BYTEDANCE_METRICS=$WITH_BYTEDANCE_METRICS \
