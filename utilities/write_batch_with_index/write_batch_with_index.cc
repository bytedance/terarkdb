...
Status WriteBatchWithIndex::GetFromBatchAndDB(DB* db,
  ...
  assert(value != nullptr);
  LazyBuffer lazy_val(value);
  auto s = GetFromBatchAndDB(db, read_options, column_family, key, &lazy_val);
  if (s.ok()) {
    s = std::move(lazy_val).dump(value);
  }
  return s;
...
Status WriteBatchWithIndex::GetFromBatchAndDB(
    ...
    const Slice& key, LazyBuffer* lazy_val, ReadCallback* callback) {
  Status s;
  MergeContext merge_context;
  ...
  if (s.ok() || s.IsNotFound()) {  // DB Get Succeeded
    if (result == WriteBatchWithIndexInternal::Result::kMergeInProgress) {
      ...
      if (merge_operator) {
        ...
        if (s.ok()) {
          s = std::move(merge_result).dump(*lazy_val);
        }
        if (s.ok()) {
          lazy_val->pin(LazyBufferPinLevel::DB);
          s = lazy_val->fetch();
