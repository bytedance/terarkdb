...
CompactionFilter::Decision FlinkCompactionFilter::ListDecide(
    const Slice& existing_value, LazyBuffer* new_value) const {
  std::size_t offset = 0;
...
void FlinkCompactionFilter::SetUnexpiredListValue(const Slice& existing_value,
                                                  std::size_t offset,
  ...
  new_value->reset(Slice(new_value_char, new_value_size), true);
