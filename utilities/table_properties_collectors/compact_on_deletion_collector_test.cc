...
int main(int /*argc*/, char** /*argv*/) {
  ...
  TERARKDB_NAMESPACE::TablePropertiesCollectorFactory::Context context;
  ...
  // randomize tests
  TERARKDB_NAMESPACE::Random rnd(301);
  ...
  for (size_t test = 0; test < window_sizes.size(); ++test) {
    ...
    // Simple test
    {
      auto factory = TERARKDB_NAMESPACE::NewCompactOnDeletionCollectorFactory(
          kWindowSize, kNumDeletionTrigger);
      const int kSample = 10;
      for (int delete_rate = 0; delete_rate <= kSample; ++delete_rate) {
        std::unique_ptr<TERARKDB_NAMESPACE::TablePropertiesCollector> collector(
        ...
        for (int i = 0; i < kPaddedWindowSize; ++i) {
          if (i % kSample < delete_rate) {
            collector->AddUserKey("hello", "rocksdb",
                                  TERARKDB_NAMESPACE::kEntryDelete, 0, 0);
            deletions++;
          } else {
            collector->AddUserKey("hello", "rocksdb",
                                  TERARKDB_NAMESPACE::kEntryPut, 0, 0);
          }
        }
