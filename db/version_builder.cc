...
namespace {
bool BySmallestKey(FileMetaData* a,
                   FileMetaData* b,
...
struct VersionBuilderDebugger {
    ...
    void PushEdit(VersionEdit* edit) {
        ...
        deletion.insert(deletion.end(), edit_deletion.begin(),
                        edit_deletion.end());

        auto& edit_addition = edit->GetNewFiles();
        addition.insert(addition.end(), edit_addition.begin(),
                        edit_addition.end());
...
class VersionBuilder::Rep {
   ...
   public:
    Rep(const EnvOptions& env_options,
        Logger* info_log,
    ...
    void PutSst(FileMetaData* f, int level) {
        auto ib = dependence_map_.emplace(
            f->fd.GetNumber(), DependenceItem{0, 0, false, level, f, 0});
    ...
    void SetDependence(FileMetaData* f,
                       bool is_map,
                       bool is_estimation,
                       double ratio,
                       bool finish) {
        ...
        for (auto it = dependence_map_.begin(); it != dependence_map_.end();) {
            ...
            if (item.dependence_version == dependence_version_) {
                assert(inheritance_counter_.count(it->first) > 0 &&
                       inheritance_counter_.find(it->first)->second.item ==
                           &item);
                if (finish) {
                    uint64_t entry_depended =
                        std::max<uint64_t>(1, item.entry_depended);
                    entry_depended =
                        std::min(item.f->prop.num_entries, entry_depended);
                    uint64_t num_antiquation =
                        item.f->prop.num_entries - entry_depended;
                    switch (item.f->gc_status) {
                        case FileMetaData::kGarbageCollectionForbidden:
                            if (item.gc_forbidden_version !=
                                dependence_version_) {
                                if (item.f->refs > 1) {
                                    // if item.f in other versions, that
                                    // assigning item.f->gc_status to permitted
                                    // might let this item participate in GC
                                    // before current version was installed. It
                                ...
                                item.f->gc_status =
                                    FileMetaData::kGarbageCollectionPermitted;
                            }
                            break;
                        case FileMetaData::kGarbageCollectionCandidate:
                            assert(item.gc_forbidden_version !=
                                   dependence_version_);
    ...
    void CheckDependence(VersionStorageInfo* vstorage,
                         FileMetaData* f,
                         bool is_map) {
        for (auto& dependence : f->prop.dependence) {
            ...
            for (size_t i = 1; i < level_files.size(); i++) {
                ...
                if (level == 0) {
                    ...
                    if (!level_nonzero_cmp_(f1, f2)) {
                        fprintf(stderr, "L%d files are not sorted properly",
                                level);
                        abort();
                    }

                    // Make sure there is no overlap in levels > 0
                    if (vstorage->InternalComparator()->Compare(
                            f1->largest, f2->smallest) >= 0) {
                        fprintf(stderr,
    ...
    void CheckConsistencyForDeletes(VersionEdit* /*edit*/,
                                    uint64_t number,
                                    int level) {
        Init();
#ifdef NDEBUG
    ...
    void Apply(VersionEdit* edit, bool strict_sequence = true) {
        ...
        for (auto& pair : edit->GetNewFiles()) {
            ...
            if (level < num_levels_) {
                ...
                assert(level < 0 ||
                       levels_[level].count(f->fd.GetNumber()) == 0);
                PutSst(f, level);
            } else {
    ...
    void SaveTo(VersionStorageInfo* vstorage, bool strict_sequence = true) {
        ...
        for (int level = 0; level < num_levels_; level++) {
            ...
            std::sort(ordered_added_files.begin(), ordered_added_files.end(),
                      cmp);
            ...
            for (auto f : ordered_added_files) {
                ...
                if (level == 0) {
                    ...
                    read_amp[level] =
                        std::max<double>(read_amp[level], f->prop.read_amp);
                }
            }
        }
        for (auto& pair : dependence_map_) {
        ...
        vstorage->oldest_snapshot_seqnum(
            base_vstorage_->oldest_snapshot_seqnum());
        bool strict_check =
            dependence_repair_mask_.size() == 0 && strict_sequence;
        CheckConsistency(
    ...
    void LoadTableHandlers(InternalStats* internal_stats,
        ...
        std::function<void()> load_handlers_func([&]() {
            while (true) {
                ...
                auto file_read_hist =
                    level >= 0 ? internal_stats->GetFileReadHist(level)
                               : nullptr;
        ...
        std::function<void()> upgrade_func([&]() {
            while (true) {
                ...
                auto s = table_cache_->GetTableProperties(
                    env_options_, *base_vstorage_->InternalComparator(),
                    *file_meta, &properties, prefix_extractor,
                    false /*no_io */);
    ...
    auto verify = [rep](VersionStorageInfo* l,
        ...
        */
        ...
        if (l_sst.size() != r_sst.size() ||
            std::mismatch(l_sst.begin(), l_sst.end(), r_sst.begin(), eq)
                    .first != l_sst.end()) {
