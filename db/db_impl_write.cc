...
Status DBImpl::TEST_WriteWithSequence(
    const WriteOptions& write_options, WriteBatch* my_batch,
    SequenceNumber sequencenumber, WriteCallback* callback, uint64_t* log_used,
    uint64_t log_ref, bool disable_memtable, uint64_t* seq_used,
    size_t batch_cnt, PreReleaseCallback* pre_release_callback) {
  LatencyHistGuard guard(&write_latency_reporter_);
  write_qps_reporter_.AddCount(WriteBatchInternal::Count(my_batch));
  write_throughput_reporter_.AddCount(WriteBatchInternal::ByteSize(my_batch));
  ...
  WriteThread::Writer w(write_options, my_batch, callback, log_ref,
                        disable_memtable, batch_cnt, pre_release_callback);
  w.sequence = sequencenumber;  // seq init
  if (!write_options.disableWAL) {
    RecordTick(stats_, WRITE_WITH_WAL);
  }
  ...
  if (status.ok()) {
    ...
    if (status.ok()) {
      ...
      if (!parallel) {
        // w.sequence will be set inside InsertInto
        w.status = WriteBatchInternal::InsertInto(
            write_group, sequencenumber, column_family_memtables_.get(),
            &flush_scheduler_, write_options.ignore_missing_column_families,
            0 /*recovery_log_number*/, this, parallel, seq_per_batch_,
            batch_per_txn_);  // sequencenumber to current_sequence
      } else {
        SequenceNumber next_sequence = current_sequence;
        // Note: the logic for advancing seq here must be consistent with the
        // logic in WriteBatchInternal::InsertInto(write_group...) as well as
