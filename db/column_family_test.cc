...
TEST_P(ColumnFamilyTest, CreateCFRaceWithGetAggProperty) {
  ...
  TERARKDB_NAMESPACE::port::Thread thread([&] { CreateColumnFamilies({"one"}); });
...
TEST_P(ColumnFamilyTest, SameCFManualManualCompactions) {
  ...
  TERARKDB_NAMESPACE::port::Thread threads1([&] {
    CompactRangeOptions compact_options;
    compact_options.exclusive_manual_compaction = false;
    ASSERT_OK(
        db_->CompactRange(compact_options, handles_[1], nullptr, nullptr));
...
TEST_P(ColumnFamilyTest, CreateAndDropRace) {
  ...
  TERARKDB_NAMESPACE::SyncPoint::GetInstance()->SetCallBack("PersistRocksDBOptions:start",
                                                 [&](void* /*arg*/) {
  ...
  // and its comparator
  TERARKDB_NAMESPACE::port::Thread drop_cf_thread(DropSingleColumnFamily, this, 1,
                                       &comparators);
...
TEST_P(ColumnFamilyTest, LogSyncConflictFlush) {
  ...
  TERARKDB_NAMESPACE::port::Thread thread([&] { db_->SyncWAL(); });
