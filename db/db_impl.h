...
class DBImpl : public DB {
   public:
    DBImpl(const DBOptions& options,
    ...
    // If value_found is not passed in as null, then return the value if found
    // in memory. On return, if value was found, then value_found will be set to
    ...
    // REQUIRES: joined the main write queue if two_write_queues is disabled,
    // and the second write queue otherwise.
    ...
    // Returns the sequence number that is guaranteed to be smaller than or
    // equal to the sequence number of any key that could be inserted into the
    // current memtables. It can then be assumed that any write with a larger(or
    // equal) sequence number will be present in this memtable or a later
    // memtable.
    //
    ...
    Status GetLatestSequenceForKey(SuperVersion* sv,
                                   const Slice& key,
                                   bool cache_only,
                                   SequenceNumber* seq,
    ...
    // make sure not to compact any keys that would prevent a write-conflict
    // from being detected.
    ...
    bool TEST_UnableToReleaseOldestLog() {
        return unable_to_release_oldest_log_;
    }
    ...
    // We add a bool parameter to wait for unscheduledCompactions_ == 0, but
    // this is only for the special test of CancelledCompactions
    ...
    Status TEST_WriteSequence(const WriteOptions& opt,
                              const Slice& key,
                              const Slice& value,
    ...
    // Returns maximum background flushes and compactions allowed to be
    // scheduled
    ...
    // Returns the lower bound file number for SSTs that won't be deleted, even
    // if they're obsolete. This lower bound is used internally to prevent newly
    // created flush/compaction output files from being deleted before they're
    // installed. This technique avoids the need for tracking the exact numbers
    // of files pending creation, although it prevents more files than necessary
    // from being deleted.
    ...
    void FindObsoleteFiles(JobContext* job_context,
                           bool force,
    ...
    // Similar to the previous function but looks up based on a column family
    // id. nullptr will be returned if this column family no longer exists.
    // REQUIRED: this function should only be called on the write thread or if
    // the mutex is held.
    SuperVersion* GetAndRefSuperVersion(uint32_t column_family_id);

    // Un-reference the super version and clean it up if it is the last
    // reference.
    ...
    // REQUIRED: this function should only be called on the write thread or if
    // the mutex is held.  Return value only valid until next call to this
    // function or mutex is released.
    ...
    struct RecoveredTransaction {
        ...
        struct BatchInfo {
            ...
            // unprepared transactions. This is only useful for rollbacks, and
            // we can in theory just keep keyset for that.
            WriteBatch* batch_;
            // Number of sub-batches. A new sub-batch is created if txn attempts
            // to insert a duplicate key,seq to memtable. This is currently used
            // in WritePreparedTxn/WriteUnpreparedTxn.
            size_t batch_cnt_;
        };
        ...
        void AddBatch(SequenceNumber seq,
                      uint64_t log_number,
                      WriteBatch* batch,
                      size_t batch_cnt,
                      bool unprepared) {
            ...
            // Prior state must be unprepared, since the prepare batch must be
            // the last batch.
            assert(unprepared_);
    ...
    void InsertRecoveredTransaction(const uint64_t log,
        ...
        if (rtxn == recovered_transactions_.end()) {
            ...
            rtxn->second->AddBatch(seq, log, batch, batch_cnt,
                                   unprepared_batch);
        }
    ...
    static Status CreateAndNewDirectory(Env* env,
   ...
   protected:
    ...
    // batch will still be skipped from memtable during the recovery. An
    // excption to this is seq_per_batch_ mode, in which since each batch
    // already takes one seq, it is ok for the batch to write to memtable during
    // recovery as long as it only takes one sequence number: i.e., no duplicate
    // keys. In WriteCommitted it is guarnateed since disable_memtable is used
    // for prepare batch which will be written to memtable later during the
    // commit, and in WritePrepared it is guaranteed since it will be used only
    // for WAL markers which will never be written to memtable. If the commit
    // marker is accompanied with CommitTimeWriteBatch that is not written to
    // memtable as long as it has no duplicate keys, it does not violate the
    // one-seq-per-batch policy. batch_cnt is expected to be non-zero in
    // seq_per_batch mode and indicates the number of sub-patches. A sub-patch
    // is a subset of the write batch that does not have duplicate keys.
    Status WriteImpl(const WriteOptions& options,
                     WriteBatch* updates,
                     WriteCallback* callback = nullptr,
                     uint64_t* log_used = nullptr,
                     uint64_t log_ref = 0,
    ...
    // the number of sub-patches. A sub-patch is a subset of the write batch
    // that does not have duplicate keys.
    Status WriteImplWALOnly(const WriteOptions& options,
                            WriteBatch* updates,
                            WriteCallback* callback = nullptr,
                            uint64_t* log_used = nullptr,
                            uint64_t log_ref = 0,
   ...
   private:
    ...
    void DeleteObsoleteFileImpl(int job_id,
    ...
    // ReleaseFileNumberFromPendingOutputs() can now be deleted (if it's not
    // live and blocked by any other pending_outputs_ calls)
    ...
    struct BGFlushArg {
        ...
        // Maximum ID of memtable to flush. In this column family, memtables
        // with IDs smaller than this value must be flushed before this flush
        // completes.
        uint64_t max_memtable_id_;
        // Pointer to a SuperVersionContext object. After flush completes,
        // RocksDB installs a new superversion for the column family. This
        // operation requires a SuperVersionContext object (currently embedded
        // in JobContext).
    ...
    Status FlushMemTablesToOutputFiles(
        const autovector<BGFlushArg>& bg_flush_args,
        bool* made_progress,
        JobContext* job_context,
        LogBuffer* log_buffer);
    ...
    Status WriteLevel0TableForRecovery(int job_id,
                                       ColumnFamilyData* cfd,
                                       MemTable* mem,
                                       VersionEdit* edit);
    ...
    // Force current memtable contents to be flushed.
    Status FlushMemTable(ColumnFamilyData* cfd,
                         const FlushOptions& options,
    ...
    Status WriteToWAL(const WriteBatch& merged_batch,
    ...
    // logs_, logfile_number_. Refer to the definition of each variable below
    ...
    // reads must be under either mutex_ or log_write_mutex_. Since after
    // ::Open, logfile_number_ is currently updated only in write_thread_, it
    // can be read from the same write_thread_ without any locks.
    ...
    // be accessed from the same write_thread_ without any locks. With
    // two_write_queues writes, where it can be updated in different threads,
    // read and writes are protected by log_write_mutex_ instead. This is to
    // avoid expesnive mutex_ lock during WAL write, which update log_empty_.
    bool log_empty_;
    ...
    struct LogWriterNumber {
        ...
        uint64_t number;
        // Visual Studio doesn't support deque's member to be noncopyable
        // because of a std::unique_ptr as a member.
        log::Writer* writer;  // own
        // true for some prefix of logs_
        bool getting_synced = false;
    };
    // Without two_write_queues, read and writes to alive_log_files_ are
    // protected by mutex_. However since back() is never popped, and
    // push_back() is done only from write_thread_, the same thread can access
    // the item reffered by back() without mutex_. With two_write_queues_,
    // writes are protected by locking both mutex_ and log_write_mutex_, and
    // reads must be under either mutex_ or log_write_mutex_.
    ...
    // If true, we have only one (default) column family. We use this to
    ...
    // used in 2PC to batch the prepares separately from the serial commit.
    ...
    // Note: This is to protect memtable and compaction. If the batch only
    // writes to the WAL its size need not to be included in this.
    ...
    // For each background job, pending_outputs_ keeps the current file number
    // at the time that background job started.
    // FindObsoleteFiles()/PurgeObsoleteFiles() never deletes any file that has
    // number bigger than any of the file number in pending_outputs_. Since file
    // numbers grow monotonically, this also means that pending_outputs_ is
    // always sorted. After a background job is done executing, its file number
    // is deleted from pending_outputs_, which allows PurgeObsoleteFiles() to
    // clean it up. State is protected with db mutex.
    std::list<uint64_t> pending_outputs_;

    // PurgeFileInfo is a structure to hold information of files to be deleted
    // in purge_queue_
    struct PurgeFileInfo {
        std::string fname;
    ...
    // When column family is put on this queue, we increase unscheduled_flushes_
    // and unscheduled_compactions_. When these variables are bigger than zero,
    // that means we need to schedule background threads for compaction and
    // thread. Once the background threads are scheduled, we decrease
    // unscheduled_flushes_ and unscheduled_compactions_. That way we keep track
    // of number of compaction and flush threads we need to schedule. This
    // scheduling is done in MaybeScheduleFlushOrCompaction() invariant(column
    // family present in flush_queue_ <==> ColumnFamilyData::pending_flush_ ==
    // true)
    ...
    virtual Status GetPropertiesOfTablesInRange(
        ...
        bool include_blob = true) override;
    ...
    // Increase the sequence number after writing each batch, whether memtable
    // is disabled for that or not. Otherwise the sequence number is increased
    // after writing each key into memtable. This implies that when
    // disable_memtable is set, the seq is not increased at all.
    //
    // Default: false
    ...
    // With atomic flush, each bg thread installs the result of flushing
    // multiple column families, and different threads can flush different
    // column families. It's difficult to rely on one thread to perform batch
...
extern uint64_t PrecomputeMinLogNumberToKeep(
    VersionSet* vset,
    const ColumnFamilyData& cfd_to_flush,
