...
TEST_F(RepairTest, LostManifest) {
    ...
    Put("key2", "val2");
    Flush();
    // Need to get path before Close() deletes db_, but delete it after Close()
...
TEST_F(RepairTest, CorruptManifest) {
    ...
    Put("key2", "val2");
    Flush();
    // Need to get path before Close() deletes db_, but overwrite it after
    // Close() to ensure Close() didn't change the manifest.
    std::string manifest_path =
        DescriptorFileName(dbname_, dbfull()->TEST_Current_Manifest_FileNo());
...
TEST_F(RepairTest, RepairMultipleColumnFamilies) {
    ...
    // Need to get path before Close() deletes db_, but delete it after Close()
    // to ensure Close() doesn't re-create the manifest.
    std::string manifest_path =
        DescriptorFileName(dbname_, dbfull()->TEST_Current_Manifest_FileNo());
    Close();
    ASSERT_OK(env_->FileExists(manifest_path));
...
TEST_F(RepairTest, RepairColumnFamilyOptions1) {
    ...
    ASSERT_OK(TryReopenWithColumnFamilies(
        {"default", "reverse"}, std::vector<Options>{opts, rev_opts}));
    for (int i = 0; i < kNumCfs; ++i) {
        for (int j = 0; j < kEntriesPerCf; ++j) {
            ASSERT_EQ(Get(i, "key" + ToString(j)), "val" + ToString(j));
        }
...
TEST_F(RepairTest, RepairColumnFamilyOptions2) {
    ...
    TablePropertiesCollection fname_to_props;
    db_->GetPropertiesOfAllTables(handles_[1], &fname_to_props);
    ASSERT_EQ(fname_to_props.size(), 3U);
    for (const auto& fname_and_props : fname_to_props) {
        std::string comparator_name(
            InternalKeyComparator(rev_opts.comparator).Name());
        comparator_name = comparator_name.substr(comparator_name.find(':') + 1);
...
TEST_F(RepairTest, RepairColumnFamilyOptions3) {
    // Verify repair logic uses correct ColumnFamilyOptions when repairing a
    // database with different options for column families.
    const int kNumCfs = 2;
    const int kEntriesPerCf = 2;
...
TEST_F(RepairTest, RepairColumnFamilyOptions4) {
    // Verify repair logic uses correct ColumnFamilyOptions when repairing a
    // database with different options for column families.
    const int kNumCfs = 2;
    const int kEntriesPerCf = 2;
...
TEST_F(RepairTest, RepairColumnFamilyOptions5) {
    // Verify repair logic uses correct ColumnFamilyOptions when repairing a
    // database with different options for column families.
    const int kNumCfs = 2;
    const int kEntriesPerCf = 2;
