...
// [file_numbers]           block_sz * uint64_t (variant)
...
class LinkBlockRecord {
 public:
  ...
  static uint32_t hash(const Slice& user_key, int max_bits) { return 0; }

 public:
  // @param key The largest internal key of all items in current LinkBlock
  // @param group_sz Total number of items inside a LinkBlock
  LinkBlockRecord(IteratorCache* iterator_cache, const Slice& key, int group_sz,
                  int hash_bits)
  ...
  void Encode(std::vector<uint64_t> file_numbers,
              std::vector<uint32_t> hash_values, const Slice& smallest_key) {
    // TODO remember to set file_number

    encoded_value = LazyBuffer();
    return;
  }
  ...
  bool DecodeFrom(Slice& input) {
    ...
    for (int i = 0; i < group_sz_; ++i) {
      ...
      uint32_t hash = 0;
      memcpy((char*)&hash, input.data() + start_bytes,
  ...
  Status Seek(const Slice& target, const InternalKeyComparator& icomp) {
    ...
    if (left < group_sz_) {
      ...
      std::set<uint64_t> unique_file_numbers(file_numbers_.begin(),
                                             file_numbers_.end());
  ...
  const Slice& MaxKey() const { return max_key_; }

  const LazyBuffer& EncodedValue() const { return encoded_value; }

  // If we have a non-empty `smallest_key_`, we should re-init all underlying
  // iterators (default: invalid)
  const Slice& SmallestKey() const { return smallest_key_; }
 ...
 private:
  ...
  // Get target key from cache. If it's not present, fetch from iterator & cache
  // it.
 ...
 private:
  ...
  Slice max_key_;
  // Encoded LBR value (contains all current LBR's information)
  LazyBuffer encoded_value;
...
class LinkSstIterator : public InternalIterator {
 ...
 private:
  ...
  bool InitLinkBlockRecords() {
    ...
    while (link_sst_iter_->Valid()) {
      ...
      lbr_list_.emplace_back(&iterator_cache_, link_sst_iter_->key(),
 ...
 public:
  ...
  void Seek(const Slice& target) override {
    ...
    auto it =
        std::lower_bound(lbr_list_.begin(), lbr_list_.end(), target,
  ...
  void Prev() override {
    ...
    while (cur_lbr_idx_ >= 0) {
      ...
      // If we cannot `Prev()` current LBR, try previous one, note that if
      // current LBR has a valid smallest key, we should re-seek previous LBR.
      cur_lbr_idx_--;
