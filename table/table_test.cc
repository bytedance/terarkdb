...
class BlockConstructor : public Constructor {
 public:
  ...
  virtual InternalIterator* NewIterator(
    ...
    return new ConvertingIterator<false, InternalIteratorBase<Slice>>(
        block_->NewIterator<DataBlockIter>(comparator_, comparator_));
  }
...
class TableConstructor : public Constructor {
 public:
  ...
  virtual Status FinishImpl(const Options& options,
    ...
    for (const auto& kv : kv_map) {
      if (convert_to_internal_key_) {
        ...
        EXPECT_TRUE(builder->Add(encoded, LazyBuffer(kv.second)).ok());
      } else {
  ...
  virtual InternalIterator* NewIterator(
    ...
    if (convert_to_internal_key_) {
      return new ConvertingIterator<true, InternalIterator>(iter);
    } else {
      return iter;
    }
  }
 ...
 public:
  ...
  virtual Status FinishImpl(
    ...
    memtable_ = new MemTable(
        internal_comparator_, mem_ioptions, MutableCFOptions(options_),
        /* needs_dup_key_check */ false, write_buffer_manager_,
  ...
  virtual InternalIterator* NewIterator(
      const SliceTransform* /*prefix_extractor*/) const override {
    return new ConvertingIterator<true, InternalIterator>(
...
class DBConstructor : public Constructor {
 public:
  ...
  virtual InternalIterator* NewIterator(
      const SliceTransform* /*prefix_extractor*/) const override {
    return new ConvertingIterator<false, Iterator>(
...
// Plain table is not supported in ROCKSDB_LITE
  ...
  std::string ToString(const InternalIterator* it) {
    if (!it->Valid()) {
      ...
      auto s = v.fetch();
      if (s.ok()) {
        return "'" + it->key().ToString() + "->" + v.ToString() + "'";
      } else {
        return "'" + it->key().ToString() + "->" + s.ToString() + "'";
      }
    }
  }
...
TEST_P(BlockBasedTableTest, SkipPrefixBloomFilter) {
  ...
  for (auto& kv : kvmap) {
    ...
    auto v = db_iter->value();
    ASSERT_OK(v.fetch());
    ASSERT_EQ(v.slice(), kv.second);
  }
}
            ...
            for (bool pin_l0 : {true, false}) {
              for (bool pin_top_level : {true, false}) {
                ...
                table_options.block_cache =
                    std::shared_ptr<TERARKDB_NAMESPACE::Cache>(
                        new MockCache(16 * 1024 * 1024, 4, false, 0.0));
                table_options.filter_policy.reset(
                    TERARKDB_NAMESPACE::NewBloomFilterPolicy(
                        10, block_based_filter));
                ...
                // There must be some pinned data since LazyBuffer has not
                // released them yet
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (i == 0) {
      ...
      typedef ConvertingIterator<false, FragmentedRangeTombstoneIterator>
          IterWrapperType;
      auto range_tombstone_iterator = memtable->NewRangeTombstoneIterator(
...
TEST_P(IndexBlockRestartIntervalTest, IndexBlockRestartInterval) {
  ...
  for (db_iter->SeekToFirst(); db_iter->Valid(); db_iter->Next()) {
    ...
    auto v = db_iter->value();
    ASSERT_OK(v.fetch());
    ASSERT_EQ(v.slice(), kv_iter->second);
    kv_iter++;
  }
...
class TestPrefixExtractor : public TERARKDB_NAMESPACE::SliceTransform {
 public:
  ...
  TERARKDB_NAMESPACE::Slice Transform(
      const TERARKDB_NAMESPACE::Slice& src) const override {
    assert(IsValid(src));
  ...
  bool InRange(const TERARKDB_NAMESPACE::Slice& /*dst*/) const override {
    return true;
  }
...
TEST_P(BlockBasedTableTest, PropertiesMetaBlockLast) {
  ...
  std::unique_ptr<InternalIteratorBase<Slice>> metaindex_iter{
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (auto& kv : kvmap) {
      if (i == 0) {
        ...
        ASSERT_OK(value.fetch());
        ASSERT_EQ(get_context.State(), GetContext::kFound);
        ASSERT_EQ(value.slice(), Slice(kv.second));
        value.clear();
      }
    }
