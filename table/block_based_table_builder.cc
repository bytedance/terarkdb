...
FilterBlockBuilder* CreateFilterBlockBuilder(
  ...
  if (filter_bits_builder == nullptr) {
    ...
    if (table_opt.partition_filters) {
      terark_assert(p_index_builder != nullptr);
      // Since after partition cut request from filter builder it takes time
      // until index builder actully cuts the partition, we take the lower bound
      // as partition size.
      terark_assert(table_opt.block_size_deviation <= 100);
      auto partition_size =
...
Status BlockBasedTableBuilder::Add(const Slice& key,
  ...
  if (r->props.num_entries > 0 &&
    ...
    terark_assert(r->internal_comparator.Compare(key, Slice(r->last_key)) > 0);
    return Status::Corruption("BlockBasedTableBuilder::Add: overlapping key");
  }

  auto should_flush = r->flush_block_policy->Update(key, value);
  if (should_flush) {
    terark_assert(!r->data_block.empty());
    Flush();
...
Status BlockBasedTableBuilder::AddTombstone(const Slice& key,
  ...
  const Slice& value = lazy_value.slice();
  terark_assert(ExtractValueType(key) == kTypeRangeDeletion);
  ...
  if (raw_block_contents.size() < kCompressionSizeLimit) {
    ...
    if (is_data_block && r->compression_dict && r->compression_dict->size()) {
      ...
      if (r->table_options.verify_compression) {
        terark_assert(r->verify_ctx != nullptr);
...
void BlockBasedTableBuilder::WriteFilterBlock(
  ...
  if (ok() && !empty_filter_block) {
    ...
    while (ok() && s.IsIncomplete()) {
      ...
      terark_assert(s.ok() || s.IsIncomplete());
...
void BlockBasedTableBuilder::WriteIndexBlock(
  ...
  if (index_builder_status.IsIncomplete()) {
    ...
    terark_assert(index_blocks.meta_blocks.empty());
...
void BlockBasedTableBuilder::WritePropertiesBlock(
  ...
  if (ok()) {
    ...
    if (rep_->table_options.index_type ==
        BlockBasedTableOptions::kTwoLevelIndexSearch) {
      terark_assert(rep_->p_index_builder_ != nullptr);
...
Status BlockBasedTableBuilder::Finish(
  ...
  if (ok()) {
    ...
    terark_assert(r->table_options.checksum == kCRC32c ||
