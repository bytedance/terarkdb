...
void test_il256_il256_uint(DataStored dtype) {
  ...
  assert(index->Name() ==
         string("CompositeUintIndex_IL_256_32_IL_256_32_BigUint"));
  printf("\tbuild done\n");
  // save & reload
...
void test_allone_il256_uint(DataStored dtype) {
  ...
  TerarkIndex* index = factory->Build(tempKeyFileReader, tableOpt, stat);
  assert(index->Name() ==
         string("CompositeUintIndex_AllOne_IL_256_32_BigUint"));
  printf("\tbuild done\n");
  // save & reload
  index_path = "./tmp_index.txt";
  index = save_reload(index, factory);
...
void test_allone_allzero_uint(DataStored dtype) {
  ...
  TERARKDB_NAMESPACE::TerarkZipTableOptions tableOpt;
  TerarkIndex* index = factory->Build(tempKeyFileReader, tableOpt, stat);
  assert(index->Name() == string("CompositeUintIndex_AllOne_AllZero_BigUint"));
  printf("\tbuild done\n");
  // save & reload
  index_path = "./tmp_index.txt";
  index = save_reload(index, factory);
...
void test_data_seek_short_target_uint() {
  ...
  TerarkIndex* index = factory->Build(tempKeyFileReader, tableOpt, stat);
  // assert(index->Name() ==
  // string("CompositeUintIndex_FewZero32_AllZero_BigUint"));
  assert(index->Name() ==
         string("CompositeUintIndex_IL_256_32_AllZero_BigUint"));
  printf("\tbuild done\n");
  // save & reload
  index_path = "./tmp_index.txt";
  index = save_reload(index, factory);
...
void test_seek_cost_effective() {
  ...
  // prepare
  const TERARKDB_NAMESPACE::TerarkIndex::Factory* f_il85_il85 =
      TerarkIndex::GetFactory("CompositeUintIndex_IL_256_32_IL_256_32");
  const TERARKDB_NAMESPACE::TerarkIndex::Factory* f_se96_se96 =
      TerarkIndex::GetFactory("CompositeUintIndex_SE_512_64_SE_512_64");
  const TERARKDB_NAMESPACE::TerarkIndex::Factory* f_allone_allzero =
...
void test_temp() {
  ...
  stat.maxKey.assign(arr, arr + KEY_LEN);
  auto factory =
      TerarkIndex::SelectFactory(stat, "NestLoudsTrieDAWG_Mixed_XL_256_32_FL");
  assert(factory != nullptr);
  printf("\tdone\n");
}
