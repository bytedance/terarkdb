...
class MyMerge : public TERARKDB_NAMESPACE::MergeOperator {
 public:
  virtual bool FullMergeV2(const MergeOperationInput& merge_in,
    ...
    for (const TERARKDB_NAMESPACE::Slice& m : merge_in.operand_list) {
...
class MyFilter : public TERARKDB_NAMESPACE::CompactionFilter {
 public:
  ...
  bool FilterMergeOperand(int level, const TERARKDB_NAMESPACE::Slice& key,
                          const TERARKDB_NAMESPACE::Slice& existing_value) const override {
...
int main() {
  ...
  status = TERARKDB_NAMESPACE::DB::Open(options, "/tmp/rocksmergetest", &raw_db);
  assert(status.ok());
  std::unique_ptr<TERARKDB_NAMESPACE::DB> db(raw_db);

  TERARKDB_NAMESPACE::WriteOptions wopts;
