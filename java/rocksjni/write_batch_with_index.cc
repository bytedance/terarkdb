...
jlong Java_org_rocksdb_WriteBatchWithIndex_newWriteBatchWithIndex__JBIZ(
  ...
  switch (jcomparator_type) {
    // JAVA_COMPARATOR
    case 0x0:
      fallback_comparator = reinterpret_cast<TERARKDB_NAMESPACE::ComparatorJniCallback*>(
    ...
    case 0x2:
      fallback_comparator = reinterpret_cast<TERARKDB_NAMESPACE::Comparator*>(
          jfallback_index_comparator_handle);
      break;
  }
  auto* wbwi = new TERARKDB_NAMESPACE::WriteBatchWithIndex(
...
void Java_org_rocksdb_WriteBatchWithIndex_put__J_3BI_3BI(
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  assert(wbwi != nullptr);
  auto put = [&wbwi](TERARKDB_NAMESPACE::Slice key, TERARKDB_NAMESPACE::Slice value) {
    return wbwi->Put(key, value);
  };
  std::unique_ptr<TERARKDB_NAMESPACE::Status> status = TERARKDB_NAMESPACE::JniUtil::kv_op(
      put, env, jobj, jkey, jkey_len, jentry_value, jentry_value_len);
  if (status != nullptr && !status->ok()) {
    TERARKDB_NAMESPACE::RocksDBExceptionJni::ThrowNew(env, status);
  }
}
...
void Java_org_rocksdb_WriteBatchWithIndex_put__J_3BI_3BIJ(
  ...
  auto put = [&wbwi, &cf_handle](TERARKDB_NAMESPACE::Slice key, TERARKDB_NAMESPACE::Slice value) {
...
void Java_org_rocksdb_WriteBatchWithIndex_merge__J_3BI_3BI(
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  assert(wbwi != nullptr);
  auto merge = [&wbwi](TERARKDB_NAMESPACE::Slice key, TERARKDB_NAMESPACE::Slice value) {
...
void Java_org_rocksdb_WriteBatchWithIndex_merge__J_3BI_3BIJ(
  ...
  auto* cf_handle = reinterpret_cast<TERARKDB_NAMESPACE::ColumnFamilyHandle*>(jcf_handle);
  assert(cf_handle != nullptr);
  auto merge = [&wbwi, &cf_handle](TERARKDB_NAMESPACE::Slice key, TERARKDB_NAMESPACE::Slice value) {
...
void Java_org_rocksdb_WriteBatchWithIndex_delete__J_3BI(JNIEnv* env,
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  assert(wbwi != nullptr);
  auto remove = [&wbwi](TERARKDB_NAMESPACE::Slice key) { return wbwi->Delete(key); };
  std::unique_ptr<TERARKDB_NAMESPACE::Status> status =
      TERARKDB_NAMESPACE::JniUtil::k_op(remove, env, jobj, jkey, jkey_len);
  if (status != nullptr && !status->ok()) {
    TERARKDB_NAMESPACE::RocksDBExceptionJni::ThrowNew(env, status);
  }
}
...
void Java_org_rocksdb_WriteBatchWithIndex_delete__J_3BIJ(
  ...
  auto* cf_handle = reinterpret_cast<TERARKDB_NAMESPACE::ColumnFamilyHandle*>(jcf_handle);
  assert(cf_handle != nullptr);
  auto remove = [&wbwi, &cf_handle](TERARKDB_NAMESPACE::Slice key) {
...
void Java_org_rocksdb_WriteBatchWithIndex_singleDelete__J_3BI(
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  assert(wbwi != nullptr);
  auto single_delete = [&wbwi](TERARKDB_NAMESPACE::Slice key) {
    return wbwi->SingleDelete(key);
  };
  std::unique_ptr<TERARKDB_NAMESPACE::Status> status =
      TERARKDB_NAMESPACE::JniUtil::k_op(single_delete, env, jobj, jkey, jkey_len);
  if (status != nullptr && !status->ok()) {
    TERARKDB_NAMESPACE::RocksDBExceptionJni::ThrowNew(env, status);
  }
}
...
void Java_org_rocksdb_WriteBatchWithIndex_singleDelete__J_3BIJ(
  ...
  auto* cf_handle = reinterpret_cast<TERARKDB_NAMESPACE::ColumnFamilyHandle*>(jcf_handle);
  assert(cf_handle != nullptr);
  auto single_delete = [&wbwi, &cf_handle](TERARKDB_NAMESPACE::Slice key) {
...
void Java_org_rocksdb_WriteBatchWithIndex_deleteRange__J_3BI_3BI(
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  assert(wbwi != nullptr);
  auto deleteRange = [&wbwi](TERARKDB_NAMESPACE::Slice beginKey, TERARKDB_NAMESPACE::Slice endKey) {
    return wbwi->DeleteRange(beginKey, endKey);
  };
  std::unique_ptr<TERARKDB_NAMESPACE::Status> status =
      TERARKDB_NAMESPACE::JniUtil::kv_op(deleteRange, env, jobj, jbegin_key,
...
void Java_org_rocksdb_WriteBatchWithIndex_deleteRange__J_3BI_3BIJ(
  ...
  auto* cf_handle = reinterpret_cast<TERARKDB_NAMESPACE::ColumnFamilyHandle*>(jcf_handle);
  assert(cf_handle != nullptr);
  auto deleteRange = [&wbwi, &cf_handle](TERARKDB_NAMESPACE::Slice beginKey,
                                         TERARKDB_NAMESPACE::Slice endKey) {
...
void Java_org_rocksdb_WriteBatchWithIndex_putLogData(JNIEnv* env, jobject jobj,
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  assert(wbwi != nullptr);
  auto putLogData = [&wbwi](TERARKDB_NAMESPACE::Slice blob) {
    return wbwi->PutLogData(blob);
  };
  std::unique_ptr<TERARKDB_NAMESPACE::Status> status =
      TERARKDB_NAMESPACE::JniUtil::k_op(putLogData, env, jobj, jblob, jblob_len);
  if (status != nullptr && !status->ok()) {
    TERARKDB_NAMESPACE::RocksDBExceptionJni::ThrowNew(env, status);
  }
}
...
jobject Java_org_rocksdb_WriteBatchWithIndex_getWriteBatch(JNIEnv* env,
  ...
  return TERARKDB_NAMESPACE::WriteBatchJni::construct(env, wb);
}
...
jlong Java_org_rocksdb_WriteBatchWithIndex_iteratorWithBase(JNIEnv* /*env*/,
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  auto* cf_handle = reinterpret_cast<TERARKDB_NAMESPACE::ColumnFamilyHandle*>(jcf_handle);
  auto* base_iterator = reinterpret_cast<TERARKDB_NAMESPACE::Iterator*>(jbi_handle);
...
jbyteArray JNICALL Java_org_rocksdb_WriteBatchWithIndex_getFromBatch__JJ_3BI(
  ...
  auto* wbwi = reinterpret_cast<TERARKDB_NAMESPACE::WriteBatchWithIndex*>(jwbwi_handle);
  auto* dbopt = reinterpret_cast<TERARKDB_NAMESPACE::DBOptions*>(jdbopt_handle);

  auto getter = [&wbwi, &dbopt](const TERARKDB_NAMESPACE::Slice& key, std::string* value) {
    return wbwi->GetFromBatch(*dbopt, key, value);
  };

  return TERARKDB_NAMESPACE::JniUtil::v_op(getter, env, jkey, jkey_len);
}
...
jbyteArray Java_org_rocksdb_WriteBatchWithIndex_getFromBatch__JJ_3BIJ(
  ...
  auto getter = [&wbwi, &cf_handle, &dbopt](const TERARKDB_NAMESPACE::Slice& key,
                                            std::string* value) {
...
jbyteArray Java_org_rocksdb_WriteBatchWithIndex_getFromBatchAndDB__JJJ_3BI(
  ...
  auto* readopt = reinterpret_cast<TERARKDB_NAMESPACE::ReadOptions*>(jreadopt_handle);

  auto getter = [&wbwi, &db, &readopt](const TERARKDB_NAMESPACE::Slice& key,
                                       std::string* value) {
...
jbyteArray Java_org_rocksdb_WriteBatchWithIndex_getFromBatchAndDB__JJJ_3BIJ(
  ...
  auto getter = [&wbwi, &db, &cf_handle, &readopt](const TERARKDB_NAMESPACE::Slice& key,
                                                   std::string* value) {
...
jlongArray Java_org_rocksdb_WBWIRocksIterator_entry1(JNIEnv* env,
                                                     ...
                                                     jlong handle) {
  auto* it = reinterpret_cast<TERARKDB_NAMESPACE::WBWIIterator*>(handle);
  const TERARKDB_NAMESPACE::WriteEntry& we = it->Entry();
  ...
  if (we.type == TERARKDB_NAMESPACE::kDeleteRecord ||
      we.type == TERARKDB_NAMESPACE::kSingleDeleteRecord ||
      we.type == TERARKDB_NAMESPACE::kLogDataRecord) {
    // set native handle of value slice to null if no value available
    results[2] = 0;
  } else {
    auto* value_slice = new TERARKDB_NAMESPACE::Slice(we.value.data(), we.value.size());
  ...
  if (jresults == nullptr) {
    // exception thrown: OutOfMemoryError
    if (results[2] != 0) {
      auto* value_slice = reinterpret_cast<TERARKDB_NAMESPACE::Slice*>(results[2]);
      delete value_slice;
    }
    delete key_slice;
    return nullptr;
