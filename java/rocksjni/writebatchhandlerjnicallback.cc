...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::PutCF(uint32_t column_family_id,
  ...
  if(status == nullptr) {
    return TERARKDB_NAMESPACE::Status::OK();   // TODO(AR) what to do if there is an Exception but we don't know the TERARKDB_NAMESPACE::Status?
  } else {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::MergeCF(uint32_t column_family_id,
    const Slice& key, const Slice& value) {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::DeleteCF(uint32_t column_family_id,
    const Slice& key) {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::SingleDeleteCF(uint32_t column_family_id,
    const Slice& key) {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::DeleteRangeCF(uint32_t column_family_id,
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::PutBlobIndexCF(uint32_t column_family_id,
    const Slice& key, const Slice& value) {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::MarkBeginPrepare(bool unprepare) {
#ifndef DEBUG
  (void) unprepare;
#else
  assert(!unprepare);
  ...
  if (m_env->ExceptionCheck()) {
    ...
    std::unique_ptr<TERARKDB_NAMESPACE::Status> status = TERARKDB_NAMESPACE::RocksDBExceptionJni::toCppStatus(m_env, exception);
    if (status == nullptr) {
      // unkown status or exception occurred extracting status
      m_env->ExceptionDescribe();
      return TERARKDB_NAMESPACE::Status::OK();  // TODO(AR) probably need a better error code here
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::MarkEndPrepare(const Slice& xid) {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::MarkNoop(bool empty_batch) {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::MarkRollback(const Slice& xid) {
...
TERARKDB_NAMESPACE::Status WriteBatchHandlerJniCallback::MarkCommit(const Slice& xid) {
...
std::unique_ptr<TERARKDB_NAMESPACE::Status> WriteBatchHandlerJniCallback::kv_op(const Slice& key, const Slice& value, std::function<void(jbyteArray, jbyteArray)> kvFn) {
  ...
  // all OK
  return std::unique_ptr<TERARKDB_NAMESPACE::Status>(new TERARKDB_NAMESPACE::Status(TERARKDB_NAMESPACE::Status::OK()));
}

std::unique_ptr<TERARKDB_NAMESPACE::Status> WriteBatchHandlerJniCallback::k_op(const Slice& key, std::function<void(jbyteArray)> kFn) {
    const jbyteArray j_key = JniUtil::copyBytes(m_env, key);
  if (j_key == nullptr) {
    // exception thrown
    if (m_env->ExceptionCheck()) {
