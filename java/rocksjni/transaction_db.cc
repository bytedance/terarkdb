...
jlong Java_org_rocksdb_TransactionDB_open__JJLjava_lang_String_2(
  ...
  auto* txn_db_options =
      reinterpret_cast<TERARKDB_NAMESPACE::TransactionDBOptions*>(jtxn_db_options_handle);
  TERARKDB_NAMESPACE::TransactionDB* tdb = nullptr;
  ...
  TERARKDB_NAMESPACE::Status s =
      TERARKDB_NAMESPACE::TransactionDB::Open(*options, *txn_db_options, db_path, &tdb);
...
jlongArray Java_org_rocksdb_TransactionDB_open__JJLjava_lang_String_2_3_3B_3J(
  ...
  TERARKDB_NAMESPACE::TransactionDB* tdb = nullptr;
  const TERARKDB_NAMESPACE::Status s = TERARKDB_NAMESPACE::TransactionDB::Open(
...
jlong Java_org_rocksdb_TransactionDB_beginTransaction__JJ(
  ...
  TERARKDB_NAMESPACE::Transaction* txn = txn_db->BeginTransaction(*write_options);
...
jlong Java_org_rocksdb_TransactionDB_beginTransaction__JJJ(
  ...
  auto* write_options =
      reinterpret_cast<TERARKDB_NAMESPACE::WriteOptions*>(jwrite_options_handle);
  auto* txn_options =
      reinterpret_cast<TERARKDB_NAMESPACE::TransactionOptions*>(jtxn_options_handle);
...
jlong Java_org_rocksdb_TransactionDB_beginTransaction_1withOld__JJJ(
  ...
  TERARKDB_NAMESPACE::TransactionOptions txn_options;
  TERARKDB_NAMESPACE::Transaction* txn =
...
jlong Java_org_rocksdb_TransactionDB_getTransactionByName(JNIEnv* env,
  ...
  TERARKDB_NAMESPACE::Transaction* txn = txn_db->GetTransactionByName(name);
...
jobject Java_org_rocksdb_TransactionDB_getLockStatusData(JNIEnv* env,
                                                         ...
                                                         jlong jhandle) {
  auto* txn_db = reinterpret_cast<TERARKDB_NAMESPACE::TransactionDB*>(jhandle);
  const std::unordered_multimap<uint32_t, TERARKDB_NAMESPACE::KeyLockInfo>
      lock_status_data = txn_db->GetLockStatusData();
  const jobject jlock_status_data = TERARKDB_NAMESPACE::HashMapJni::construct(
  ...
  const TERARKDB_NAMESPACE::HashMapJni::FnMapKV<const int32_t, const TERARKDB_NAMESPACE::KeyLockInfo>
      fn_map_kv =
          [env](
              const std::pair<const int32_t, const TERARKDB_NAMESPACE::KeyLockInfo>&
                  pair) {
            const jobject jlong_column_family_id =
                TERARKDB_NAMESPACE::LongJni::valueOf(env, pair.first);
            if (jlong_column_family_id == nullptr) {
              // an error occurred
              return std::unique_ptr<std::pair<jobject, jobject>>(nullptr);
            }
            const jobject jkey_lock_info =
                TERARKDB_NAMESPACE::KeyLockInfoJni::construct(env, pair.second);
  ...
  if (!TERARKDB_NAMESPACE::HashMapJni::putAll(env, jlock_status_data,
...
jobjectArray Java_org_rocksdb_TransactionDB_getDeadlockInfoBuffer(
  ...
  jobjectArray jdeadlock_info_buffer =
      env->NewObjectArray(deadlock_info_buffer_len,
                          TERARKDB_NAMESPACE::DeadlockPathJni::getJClass(env), nullptr);
  ...
  for (auto buf_it = deadlock_info_buffer.begin(); buf_it != buf_end;
       ++buf_it) {
    const TERARKDB_NAMESPACE::DeadlockPath deadlock_path = *buf_it;
    const std::vector<TERARKDB_NAMESPACE::DeadlockInfo> deadlock_infos =
        deadlock_path.path;
    const jsize deadlock_infos_len =
        static_cast<jsize>(deadlock_info_buffer.size());
    jobjectArray jdeadlock_infos = env->NewObjectArray(
        deadlock_infos_len, TERARKDB_NAMESPACE::DeadlockInfoJni::getJClass(env), nullptr);
    ...
    for (auto infos_it = deadlock_infos.begin(); infos_it != infos_end;
         ++infos_it) {
      const TERARKDB_NAMESPACE::DeadlockInfo deadlock_info = *infos_it;
      const jobject jdeadlock_info = TERARKDB_NAMESPACE::TransactionDBJni::newDeadlockInfo(
    ...
    const jobject jdeadlock_path = TERARKDB_NAMESPACE::DeadlockPathJni::construct(
