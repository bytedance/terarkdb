...
// calling C++ TERARKDB_NAMESPACE::EnvOptions methods
// from Java side.
...
jlong Java_org_rocksdb_EnvOptions_newEnvOptions(JNIEnv * /*env*/,
                                                jclass /*jcls*/) {
  auto *env_opt = new TERARKDB_NAMESPACE::EnvOptions();
...
void Java_org_rocksdb_EnvOptions_disposeInternal(JNIEnv * /*env*/,
                                                 jobject /*jobj*/,
                                                 jlong jhandle) {
  auto *eo = reinterpret_cast<TERARKDB_NAMESPACE::EnvOptions *>(jhandle);
  assert(eo != nullptr);
  delete eo;
}
...
void Java_org_rocksdb_EnvOptions_setRateLimiter(JNIEnv * /*env*/,
  ...
  auto *sptr_rate_limiter =
      reinterpret_cast<std::shared_ptr<TERARKDB_NAMESPACE::RateLimiter> *>(rl_handle);
  auto *env_opt = reinterpret_cast<TERARKDB_NAMESPACE::EnvOptions *>(jhandle);
