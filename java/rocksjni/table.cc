...
jlong Java_org_rocksdb_PlainTableConfig_newTableFactoryHandle(
  ...
  TERARKDB_NAMESPACE::PlainTableOptions options = TERARKDB_NAMESPACE::PlainTableOptions();
  ...
  options.encoding_type = static_cast<TERARKDB_NAMESPACE::EncodingType>(jencoding_type);
  options.full_scan_mode = jfull_scan_mode;
  options.store_index_in_file = jstore_index_in_file;
  return reinterpret_cast<jlong>(TERARKDB_NAMESPACE::NewPlainTableFactory(options));
}
...
jlong Java_org_rocksdb_BlockBasedTableConfig_newTableFactoryHandle(
  ...
  TERARKDB_NAMESPACE::BlockBasedTableOptions options;
  options.no_block_cache = no_block_cache;

  if (!no_block_cache) {
    if (jblock_cache > 0) {
      std::shared_ptr<TERARKDB_NAMESPACE::Cache> *pCache =
          reinterpret_cast<std::shared_ptr<TERARKDB_NAMESPACE::Cache> *>(jblock_cache);
      options.block_cache = *pCache;
    } else if (block_cache_size > 0) {
      if (block_cache_num_shardbits > 0) {
        options.block_cache =
            TERARKDB_NAMESPACE::NewLRUCache(block_cache_size, block_cache_num_shardbits);
      } else {
        options.block_cache = TERARKDB_NAMESPACE::NewLRUCache(block_cache_size);
      }
    }
  }
  ...
  if (jfilter_policy > 0) {
    std::shared_ptr<TERARKDB_NAMESPACE::FilterPolicy> *pFilterPolicy =
        reinterpret_cast<std::shared_ptr<TERARKDB_NAMESPACE::FilterPolicy> *>(
            jfilter_policy);
  ...
  if (block_cache_compressed_size > 0) {
    if (block_cache_compressd_num_shard_bits > 0) {
      options.block_cache = TERARKDB_NAMESPACE::NewLRUCache(
          block_cache_compressed_size, block_cache_compressd_num_shard_bits);
    } else {
      options.block_cache = TERARKDB_NAMESPACE::NewLRUCache(block_cache_compressed_size);
    }
  }
  options.checksum = static_cast<TERARKDB_NAMESPACE::ChecksumType>(jchecksum_type);
  options.index_type =
      static_cast<TERARKDB_NAMESPACE::BlockBasedTableOptions::IndexType>(jindex_type);
  options.format_version = jformat_version;

  return reinterpret_cast<jlong>(TERARKDB_NAMESPACE::NewBlockBasedTableFactory(options));
}
