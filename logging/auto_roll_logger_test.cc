...
TEST_F(AutoRollLoggerTest, LogFlushWhileRolling) {
  ...
  AutoRollLogger* auto_roll_logger =
      dynamic_cast<AutoRollLogger*>(logger.get());
  ASSERT_TRUE(auto_roll_logger);
  TERARKDB_NAMEPSACE::port::Thread flush_thread;
  ...
  TERARKDB_NAMEPSACE::SyncPoint::GetInstance()->LoadDependencyAndMarkers(
...
TEST_F(AutoRollLoggerTest, LogFileExistence) {
  TERARKDB_NAMEPSACE::DB* db;
  TERARKDB_NAMEPSACE::Options options;
#ifdef OS_WIN
  // Replace all slashes in the path so windows CompSpec does not
  // become confused
  std::string testDir(kTestDir);
  ...
  ASSERT_OK(TERARKDB_NAMEPSACE::DB::Open(options, kTestDir, &db));
  ASSERT_OK(default_env->FileExists(kLogFile));
  delete db;
}
