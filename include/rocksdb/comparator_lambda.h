...
template<class Comp>
StdComparareLessType<Comp>
StdCompareLess(const Comp* cmp) {
  return StdComparareLessType<Comp>{cmp};
}
template<class Comp>
StdComparareGreaterType<Comp>
StdCompareGreater(const Comp* cmp) {
  return StdComparareGreaterType<Comp>{cmp};
}
template<class Comp>
StdComparareEqualType<Comp>
StdCompareEqual(const Comp* cmp) {
...
decltype(cmp(SFINAE_STR,SFINAE_STR),
         Cmp3_to_Less<Cmp3>{&cmp}) {
  return Cmp3_to_Less<Cmp3>{&cmp};
}
template<class Cmp3>
auto operator>(const char(&)[1], const Cmp3& cmp) ->
decltype(cmp(SFINAE_STR,SFINAE_STR),
         Cmp3_to_Greater<Cmp3>{&cmp}) {
  return Cmp3_to_Greater<Cmp3>{&cmp};
}
template<class Cmp3>
auto operator>(const char(&)[1], const Cmp3& cmp) ->
decltype(cmp(SFINAE_STR,SFINAE_STR),
         Cmp3_to_Equal<Cmp3>{&cmp}) {
